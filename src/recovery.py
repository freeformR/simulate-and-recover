...
def recover_parameters(R_obs: float, M_obs: float, V_obs: float) -> tuple[float, float, float]:
    ...
    V_obs = max(V_obs, eps**2)  # Prevent tiny variances
    ...
    try:
        ...
        # Clip estimates to valid ranges
        ...
        # Fallback to defaults if analytical initialization fails
        pass

    # Optimization with proper parameter ordering
    def loss(params):
        try:
            a, v, t = params
